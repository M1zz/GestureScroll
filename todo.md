...
- [x] Mouse 클릭 제스처 변경: ✊ 주먹 → 🤏 핀치만 (보자기에서 움직임 최소, 클릭 시 커서 안 밀림)
- [x] 모드 세그먼트 정렬 정리: 세그먼트 내 커스텀 폰트 제거 + labelsHidden + maxWidth
  - (정확한 증상 재확인 필요 — "스크롤만 세그먼트가 가운데 안 옴"이 계속되면 스크린샷 요청)
