...
struct HelpView: View {
    ...
    private var modesSection: some View {
        VStack(alignment: .leading, spacing: 12) {
            ...
            modeCard("Mouse (커서)",
                     "손을 움직이면 마우스 커서가 화면 위를 따라다닙니다(카메라 프레임 안쪽 70%가 전체 화면에 대응). 🤏 엄지와 검지를 살짝 집으면 누르고 놓으면 뗍니다 — 집었다 놓기 = 클릭, 집은 채 움직이면 드래그. 손 전체를 쥐는 동작이 아니라서 클릭할 때 커서가 밀리지 않습니다. 손바닥 ✋으로 Listening을 켠 뒤 사용하세요.")
        }
    }
